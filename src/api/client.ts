/**
 * The Fountain API, as much of it as Table Talk needs. Every call carries the
 * bearer key; every error is an `ApiError` with the server's `error` string
 * when there was one. Ported (slimmed) from jhgaylor/dns-desk.
 */
import type { Agent, Conversation, LogEvent, Teammate, Turn } from "./types";
...
export class FountainClient {
  ...
  /** Fresh thread on the same computer — how each dataset gets its own conversation. */
  startFreshThread(agentId: string): Promise<unknown> {
    return this.json("POST", `/api/team/${agentId}/conversations`);
  }

  // ── set-up: creating the analyst agent ────────────────────────────────────

  async findAgents(name: string): Promise<Agent[]> {
    const qs = new URLSearchParams({ search: name });
  ...
  /** Keep a reused agent's operating rules current with this build's spec. */
  updateAgent(agentId: string, input: { description?: string; system?: string }): Promise<Agent> {
    return this.json<{ data: Agent }>("PUT", `/api/agents/${agentId}`, input).then((r) => r.data);
  }

  // ── the conversations datasets live in ────────────────────────────────────
  ...
  if (err instanceof ApiError) {
    switch (err.code) {
      case "conversation_busy":
        return "The analyst is still working on the last question — one moment.";
      case "provisioning":
        return "The analyst's computer is still starting — try again shortly.";
      case "conversation_terminated":
        return "That analysis has ended — drop the file again to start fresh.";
