...
a `+` menu — never an agent-and-environment form. `README.md` has the
product write-up; this file has what you need to change it safely.
...
server/   Bun. app.ts is the route table; index.ts boots it.
          ...
          presets.ts   GET /api/me/presets: the caller's agents/envs/vaults + catalog
...
shared/   what both sides agree on: author.ts, models.ts, settings.ts, images.ts
...
2. **Settings → agent (`server/agents.ts`).** A preset on its own
   runtime+model is used as-is. Otherwise an agent is derived once and found
   again by `metadata.salon.key` (`shared/settings.ts#derivedKey`). Change
   the key shape and every existing derived agent is orphaned — the host's
   agent list grows by one per pick until they are cleaned up.
   `presets.ts` hides agents carrying `metadata.salon`; keep that or the
   menu offers `Salon · …` agents as presets.
...
bun server/index.ts`, then sign in with curl
...
and streams in about a minute. Retire the chat afterwards (⋯ → Retire) so
