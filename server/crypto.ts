 ...
 * a key derived from `WORKBENCH_SECRET`, so a copy of the database alone is
 * not a copy of everyone's Fountain access. Session tokens are stored only
 * as SHA-256 hashes for the same reason.
 */
...
export class Cipher {
  ...
  static async from(secret: string): Promise<Cipher> {
    if (secret.length < 16) throw new Error("WORKBENCH_SECRET must be at least 16 characters");
