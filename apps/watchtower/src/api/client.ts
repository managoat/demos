/**
 * The Fountain API, as much of it as Watchtower needs. Every call carries the
 * bearer key; every error is an `ApiError` with the server's `error` string
 * when there was one. Ported (slimmed) from jhgaylor/dns-desk, plus the team
 * schedules endpoints — the schedule is this app's heartbeat.
 */
import type { Agent, Catalog, LogEvent, Schedule, Teammate, Turn } from "./types";
...
export class FountainClient {
  ...
  // ── schedules: the patrol heartbeat ───────────────────────────────────────

  async listSchedules(agentId: string): Promise<Schedule[]> {
    return (await this.json<{ data: Schedule[] }>("GET", `/api/team/${agentId}/schedules`)).data;
  }

  async createSchedule(agentId: string, input: { cron: string; prompt: string; name?: string; one_off?: boolean }): Promise<Schedule> {
    return (await this.json<{ data: Schedule }>("POST", `/api/team/${agentId}/schedules`, input)).data;
  }

  async updateSchedule(agentId: string, id: string, patch: Partial<Pick<Schedule, "cron" | "prompt" | "name" | "enabled">>): Promise<Schedule> {
    return (await this.json<{ data: Schedule }>("PATCH", `/api/team/${agentId}/schedules/${id}`, patch)).data;
  }
  ...
  // ── the conversation the tower runs on ────────────────────────────────────
  ...
  // ── set-up: creating the tower agent ──────────────────────────────────────
  ...
  if (err instanceof ApiError) {
    switch (err.code) {
      case "conversation_busy":
        return "Watchtower is mid-patrol — one moment.";
      case "provisioning":
        return "Watchtower's computer is still starting — try again shortly.";
