// Shapes as served by the Fountain API, narrowed to what drydock reads.
...
export interface Sandbox {
   ...
   * three to attach with `sandbox_id`; this is the whole reason drydock keeps
...
export interface Agent {
  id: string;
  name: string;
  /** An agent's default. Drydock's is `persistent`: the machine is the point. */
 ...
 *
 * Drydock only ever reads these. Connecting one "needs a browser and a session,
 * so it is not an API operation" — it happens at Fountain's console, and the
 * most drydock can do is say whether it has happened and link to where it does.
 */
