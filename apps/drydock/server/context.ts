 ...
 * and every route downstream has to handle both halves. Drydock has one
...
/** One thread of one of the caller's projects, and the project it belongs to. */
export function threadOf(ctx: AppContext, user: UserRow, threadId: string): { thread: ThreadRow; project: ProjectRow } {
  const thread = ctx.db.thread(threadId);
  if (!thread) throw new HttpError(404, "not_found", "No such thread.");
  const project = projectOf(ctx, user, thread.projectId);
 ...
 * variable without which drydock has no machines at all.
 */
export function requireFountain(ctx: AppContext): Fountain {
  if (!ctx.fountain) {
    throw new HttpError(503, "no_fountain", "This drydock has no Fountain account configured, so it cannot build machines.");
  }
  return ctx.fountain;
}
...
export function requireGitHub(ctx: AppContext): GitHub {
  if (!ctx.github) {
    throw new HttpError(503, "no_github", "This drydock has no GitHub App configured, so it cannot see repositories.");
  }
  return ctx.github;
}
...
export function requireSprites(ctx: AppContext): Sprites {
  if (!ctx.sprites) {
    throw new HttpError(501, "no_exec", "This drydock has no Sprites token, so it cannot run commands on the machine directly.");
  }
  return ctx.sprites;
}
