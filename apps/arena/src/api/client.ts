/**
 * The Fountain API, as much of it as Arena needs. Every call carries the
 ...
 */
import type { Agent, Catalog, LogEvent, Teammate, Turn } from "./types";
...
export class FountainClient {
  ...
  // ── the conversations the columns run on ──────────────────────────────────
  ...
  if (err instanceof ApiError) {
    switch (err.code) {
      case "conversation_busy":
        return "That contender is still answering the last prompt.";
      case "provisioning":
        return "The contender's computer is still starting — hold on.";
      case "subscription_required":
        return "An active Fountain subscription is required.";
      case "sandbox_quota_exceeded":
        return "Sandbox quota reached — fewer contenders, or retire some teammates in Fountain.";
      case "not_found":
        return "Not found.";
      default:
